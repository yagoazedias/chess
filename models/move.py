...
# obs.: nao considera a captura an passant
def pawn_valid_moves(pos, board):
    moves = []
    first_move = False
    color = get_color()

    # acho q esta meio feia essa forma q implementei
    if color == 'white':
        go = up
...
board = [['bR', 'bN', 'bB', 'bQ', 'bK', 'bB', 'bN', 'bR'],
         ...
         ['-', '-', '-', '-', '-', '-', '-', '-'],
         ['wP', 'wP', 'wP', 'wP', 'wP', 'wP', 'wP', 'wP'],
         ['wR', 'wN', 'bB', 'wQ', 'wK', 'wB', 'wN', 'wR']]


t = pawn_valid_moves((5, 1), board)
