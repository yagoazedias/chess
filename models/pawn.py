...
class Pawn(Piece):
    ...
    def update_possible_moves(self, board):
        ...
        if board.is_valid_pos(diag_left_house) and board.has_oponent(diag_left_house, color):
            self.move_list.append(diag_left_house)

        if board.is_valid_pos(diag_right_house) and board.has_oponent(diag_right_house, color):
