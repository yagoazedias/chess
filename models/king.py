...
class King(Piece):
    ...
    def update_possible_moves(self, board):
        ...
        for move in self.move_list:
            if not(board.is_valid_pos(move)) or has_teammate(move, self.get_color()):
        ...
        if self.first_move and not(self.is_checked):
            # Torre da direita
            if board.get_piece((self.row, self.col+3)) == "Rook" and board.get_piece((self.row, self.col+3)).get_is_first_move()
                if board.is_empty(right(pos)) and board.is_empty(right(right(pos)))
                    self.move_list.append(right(right(pos)))
            # Torre da esquerda
            if board.get_piece((self.row, self.col-4)) == "Rook" and board.get_piece((self.row, self.col-4)).get_is_first_move()
                if board.is_empty(left(pos)) and board.is_empty(left(left(pos))) and board.is_empty(left(left(left(pos))))
